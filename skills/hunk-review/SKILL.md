...
- `<id>` -- match by exact ID (use when multiple sessions share a repo)
...
- `--repo` or `<id>` usually selects the session you want
...
- **"Multiple active sessions match"** -- pass `<id>` explicitly.
