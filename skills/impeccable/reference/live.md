> **NOT VENDORED.** Live browser mode was stripped from this skill (network + agent-driven DOM mutation surface). The instructions below describe the **upstream** flow. Do NOT run any `{{scripts_path}}/live*.mjs` command here; those scripts are not present. This file is retained for conceptual reference only. For visual inspection, use your harness's screenshot/computer-use tooling against the offline detector (`node detect/detect-antipatterns.mjs`).
