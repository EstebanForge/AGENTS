...
## Step 6: Live mode (NOT VENDORED)

**Skip this step entirely.** Live browser mode and `detect-csp.mjs` were stripped from this vendored skill. Do not write `.impeccable/live/config.json`, do not run CSP detection, and do not reference [live.md](live.md) for setup. If the user wants live iteration, they can wire up upstream impeccable themselves; this skill does not provide it.

**Nothing to configure here (live mode not vendored).**
