...
2. **Compute the slug** (persistence helper not vendored; skip persistence and trend for this run, but continue the critique):
   ```bash
   node detect/detect-antipatterns.mjs --help   # detector IS vendored; critique-storage.mjs is NOT
   ```
   Keep the resolved target path for reporting.
...
CLI scan:
```bash
node detect/detect-antipatterns.mjs --json [target]
```
...
4. **Browser visualization is NOT vendored** (live-server.mjs / live mode were stripped). Skip the browser-evidence sub-step; rely on the CLI scan and Assessment A. Do not attempt to start `live-server.mjs`.
...
2. **Persistence is NOT vendored** (`critique-storage.mjs` was stripped from this skill). Skip the write/trend/latest steps; deliver the structured critique and snapshot path in the final chat output instead. Do not attempt to invoke `critique-storage.mjs`.

3. **No temp body file is written** (persistence is not vendored), so there is nothing to delete. Skip cleanup.

4. **Trend is NOT vendored** (`critique-storage.mjs trend` was stripped). No historical trend is available. Skip the trend line in the output, or note "no trend available (persistence not vendored)" if the user expects it.
