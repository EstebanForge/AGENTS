# /impeccable hooks  (NOT VENDORED)

> **Do not follow the commands in this file.** The hook system (after-edit auto-injection of findings as system reminders) was stripped from this skill as a prompt-injection surface. `{{scripts_path}}/hook-admin.mjs` and `npx impeccable` invocations below will not resolve. The file is retained for reference only. For detector output, run it manually: `node detect/detect-antipatterns.mjs <files>`.
