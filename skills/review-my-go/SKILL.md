---
name: review-my-go
description: Review Go code against Go idioms, guidelines, practices, and mistake checklists. Use when specifically reviewing Go source files or requested Go code changes.
---

# Review My Go

Review Go changes in a git repo against the 100 Go Mistakes checklist in [go-mistakes.md](go-mistakes.md) (source: 100go.co) and the practice checks in [go-recipes.md](go-recipes.md) (source: github.com/nikolaydubina/go-recipes), then against modern-Go guidelines via the bundled CLI (source: JetBrains go-modern-guidelines). High signal: only defects present in the diff; no drive-bys on untouched code.
...
2. **Load the rubrics.** Read [go-mistakes.md](go-mistakes.md) and [go-recipes.md](go-recipes.md) from this folder. Done when: both checklists are in context.

3. **Apply.** Measure every changed file against every applicable mistake and recipe. Read surrounding source when a hunk cannot prove or clear a rule on its own. Done when: each changed file has been checked against the rubrics.
...
6. **Report.** Most impactful first. Per finding: category (Bug/Critical, Suggestion, Nit, Modernization), citation (mistake `#39`, recipe `recipe:goleak`, guideline ID `modern:rangeint`, or `go fix`), file:line or code fragment, and the fix. Note Good patterns too. Done when: every finding carries category, citation, location, and fix.

7. **Verdict.** Approve, Request Changes, or Needs Discussion, with a one-line rationale. Done when: the verdict is stated.

## Notes
- Recipe scope: a recipe applies only when the diff introduces the practice gap (new goroutine-spawning tests, new exec calls, go.mod bumps). Pre-existing gaps are drive-bys: no recipe finding on untouched code.
